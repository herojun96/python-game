...
start = False
#배경음악 재생
bg_sound = pygame.mixer.Sound(os.path.join(current_path,'music.ogg'))
    ...
    if start:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LEFT:
                    ...
                    ####무기 발사시 소리#########
                    weapon_sound = pygame.mixer.Sound(os.path.join(current_path,'weapon.ogg'))
        ...
        for item_idx, item_val in enumerate(items):
            ...
            if character_rect.colliderect(item_rect):
                ...
                #아이템 먹을시 소리
                item_sound = pygame.mixer.Sound(os.path.join(current_path,'item.ogg'))
        ...
        #무기로 enemy 적중 시 이미지 삭제 and item 먹을시 item 이미지 삭제
        if weapon_remove != -1:
